...
from ..models import Answer, Question, Quiz, User, Subject, Theory, TakenQuiz
import random
...
class TeacherSignUpView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        kwargs['user_type'] = 'преподаватель'
...
def add_theme(request):
    if request.method =='POST':
        subject = Subject()
        subject.name = request.POST.get('theme')
        subject.color = "#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)])
        subject.save()
        messages.success(request, 'Тема добавлена!')
        return HttpResponseRedirect("/teachers/quiz/theme")
    return render(request, "classroom/teachers/quiz_add_theme.html")
...
def add_theory(request):
    if request.method =='POST':
        theory = Theory()
        theory.title = request.POST.get('theme')
        theory.link = request.POST.get('link')
        theory.save()
        messages.success(request, 'Руководство добавлено!')

        return HttpResponseRedirect("/teachers/quiz/theory")
    return render(request, "classroom/teachers/add_theory.html")
...
def load_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="results.csv"; charset="cp1251"' 
